...
const MembershipDashboard = () => {
  ...
  const handleCancelMembership = async () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          // Replace with your actual backend API call to cancel membership
          // Example: await axios.post(`${API_BASE}/api/membership/cancel`, { email: user.email });
          Swal.fire(
            "Cancelled!",
            "Your membership has been cancelled.",
            "success"
          );
          refetchMembershipStatus(); // Refresh status after action
  ...
  const handleUpgradeMembership = async () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        // Redirect to your pricing or membership plans page
        // Example: navigate("/pricing-plans"); // Use useNavigate hook for actual navigation
        Swal.fire("Redirecting...", "", "info");
      }
    });
  };
  ...
  const handleDownloadPaymentHistory = () => {
    ...
    // Call autoTable as a function on the doc instance
    autoTable(doc, {
      head: [tableColumn],
      body: tableRows,
      startY: 45,
