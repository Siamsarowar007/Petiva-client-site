...
//       whileHover={{ scale: 1.02 }}
//       transition={{ duration: 0.3 }}
//       className="h-[320px] sm:h-[350px] md:h-[380px] lg:h-[400px] xl:h-[420px] flex"
//     >
...
// // 🔴 পরিবর্তন: PostCard কে named export করা হয়েছে
// export const PostCard = ({ post, handleVote, user, SITE_URL }) => { 
//   const [showShare, setShowShare] = useState(false);
//   const shareRef = useRef(null);
...
//       {/* Post Grid */}
//       {/* 🔴 পরিবর্তন: homeView এর উপর ভিত্তি করে গ্রিড কলাম */}
...
//       {/* Pagination */}
//       {/* 🔴 পরিবর্তন: homeView না হলে পেজিনেশন দেখাবে */}
//       {!homeView && (
//         <div className="flex flex-col sm:flex-row items-center justify-between max-w-6xl mx-auto px-4 py-10 gap-3">
//           <p className="text-sm text-gray-500 text-center sm:text-left">
//             Showing {currentPosts.length} of {sortedPosts.length} posts
...
// import useAxios from "../../hooks/useAxios";
// import useAuth from "../../hooks/useAuth";
// import Loader from "../../shared/Loader/Loader"; // Ensure this path is correct
...
// // PostCard Component: Now memoized and named exported
// export const PostCard = React.memo(({ post, handleVote, user, SITE_URL }) => {
//   const [showShare, setShowShare] = useState(false);
//   const shareRef = useRef(null);
...
//       className="h-[320px] sm:h-[350px] md:h-[380px] lg:h-[400px] xl:h-[420px] flex"
//     >
//       <div className="bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-shadow duration-300 border border-gray-100 p-6 h-full flex flex-col justify-between group overflow-hidden relative">
//         <Link to={`/post-details/${post._id}`} className="block flex-grow">
//           <div className="flex items-start gap-4 mb-4">
//             <img
//               className="h-14 w-14 rounded-full object-cover ring-2 ring-offset-2 ring-[#4CA3B8] shadow-md"
...
//               {showShare && (
//                 <div
//                   className="absolute right-0 bottom-full mb-3 z-10 flex items-center gap-4 bg-white border border-gray-200 rounded-xl shadow-xl p-4 transform origin-bottom-right animate-fade-in-up"
//                 >
//                   <FacebookShareButton
//                     url={shareUrl}
//                     quote={shareTitle}
...
// const AllPost = ({ homeView = false }) => {
//   // const [posts, setPosts] = useState([]); // No longer needed, data comes from useQuery
//   const [tags, setTags] = useState(["all"]);
//   // const [loading, setLoading] = useState(true); // No longer needed, useQuery's isLoading is used
...
//   const {
//     data: posts = [], // Renaming data to 'posts' and providing default empty array
//     isLoading,
//     isError,
//     error
//   } = useQuery({
//     queryKey: ['allPosts'], // Unique queryKey for these posts (shared with HomePost)
//     queryFn: async () => {
//       // Use privateAxios for authenticated calls
...
//       // Update tags state from fetched data
//       const tagSet = new Set(["all"]);
//       postsWithDetails.forEach((p) => {
//         if (p.tag) tagSet.add(p.tag);
//       });
...
//       return postsWithDetails; // Return the fetched and processed data
//     },
//     staleTime: 5 * 60 * 1000, // Data will not be stale for 5 minutes
//     cacheTime: 10 * 60 * 1000, // Data will stay in cache for 10 minutes
//     // enabled: !!user?.email, // Optional: only fetch if user is logged in
//   });
...
//         );
//       });
//       // If other components (like HomePost) also display this data,
//       // you might want to invalidate their specific query keys as well if they are different.
//       // queryClient.invalidateQueries(['homePosts']); // Example if homePosts had a separate query key
...
//   if (isLoading) { // 🔴 Using isLoading from useQuery
//     return (
//       <div className="text-center mt-24 text-xl">
//         <Loader />
//       </div>
...
//   if (isError) { // 🔴 Using isError from useQuery
...
//     <div className="min-h-screen w-full mt-16 flex flex-col justify-between px-2 md:px-0">
//       {/* Filter / Sort / Search */}
//       {!homeView && ( // 🔴 Only show this section if not in homeView
//         <div className="max-w-6xl mx-auto grid md:grid-cols-3 gap-4 items-center justify-between mb-6 px-2">
//           {/* Search */}
//           <div className="flex gap-2 items-center">
...
//       {/* Pagination */}
//       {!homeView && ( // 🔴 Only show pagination if not in homeView
//         <div className="flex flex-col sm:flex-row items-center justify-between max-w-6xl mx-auto px-4 py-10 gap-3">
//           <p className="text-sm text-gray-500 text-center sm:text-left">
//             Showing {currentPosts.length} of {sortedPosts.length} posts
//           </p>
...
const API_BASE = import.meta.env.VITE_API_URL || "http://localhost:5000";
// SITE_URL removed, as it's now handled within PostCard
...
const AllPost = ({ homeView = false }) => {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      // 🔴 FIX 1: Populate tags state from fetched data
      const fetchedTags = new Set(["all"]); // Always include "all"
      postsWithDetails.forEach((p) => {
        if (p.tag) fetchedTags.add(p.tag); // Add unique tags from posts
      });
      setTags(Array.from(fetchedTags)); // Convert Set to Array and update state
